...
namespace TestAlttrashCSharp.tests
{
    public class AdBidmachine
    {
            ...
            SetupUtils.ClickCustom(driverAppium, Path.BtnSourceAdPath);
            Thread.Sleep(2000);
            ...
            _ = driverAppium.PageSource;
                ...
                for (int i = 1; i < 12; i++)
                {
                    for (int j = 1; j < 6; j++)
                    {
                        ...
                        if (title != null && title.Equals("bidmachine", StringComparison.InvariantCultureIgnoreCase))
                        {
                            itemAd.Click();
                            isFound = true;
                            break;
                ...
                Assert.NotNull(ad);
                Assert.AreEqual(ad.ad_source.ToLower(), "bidmachine");
        ...
        [Test]
            ...
            Thread.Sleep(2000);
                ...
                var ad = SetupUtils.ReturnAdPlatformObject(list);
                Assert.NotNull(ad);
                Assert.AreEqual(ad.ad_source, "bidmachine");
                Assert.True(ad.value < 1);
            }
