        ...
        public void RV()
        {
            ...
            for (int i = 1; i < 12; i++)
            {
                for (int j = 1; j < 6; j++)
                {
                    ...
                    if (title != null && title.Equals("superawesome", StringComparison.InvariantCultureIgnoreCase))
                    {
                        itemAd.Click();
                        isFound = true;
                        break;
            ...
            Assert.NotNull(ad);
            Assert.AreEqual(ad.ad_source, "superawesome");
